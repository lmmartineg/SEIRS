function [error,residuales] = evaluate_fitness(params, U, y_exp, param_bounds)
    % Parameters to estimate: [beta, gamma, alpha, nu, B, d]
    for j = 1:size(param_bounds,1)
        params(j) = max(param_bounds(j, 1), ...
                  min(param_bounds(j, 2), params(j)));
    end
